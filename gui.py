    ...
    def lanch_parsing(self):
        try:
            ...
            if (str(self.database_path_label["text"]) != "No SQL dump selected...") and (
                ...
                nl2sql = Nl2Sql(database_path = self.database_path_label["text"],
                       lang_config_path = self.language_path_label["text"], thesaurus_path = thesaurus_path)
                query = nl2sql.get_sql_query(self.input_sentence_string.get())
                showinfo('SQL Query',query)
            else:
...
root.mainloop()
#count the number of cities with cityname as 'Allahabad'